...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (items.length === 0 && donation === 0) {
      return jsonError(400, "Empty cart");
    }
    const line_items = items.map((it) => ({
      price_data: {
        currency: "usd",
        product_data: {
          name: it.name || "BFFSA Item",
          description: it.subtitle || it.eventName || void 0,
          metadata: {
    ...
    if (donation > 0) {
      line_items.push({
        price_data: {
          currency: "usd",
          product_data: { name: "Additional Donation to BFFSA" },
          unit_amount: Math.round(donation * 100)
        },
        quantity: 1
      });
    }
    const events = [...new Set(items.map((i) => i.eventName).filter(Boolean))].join(", ");
    const meta = {
    ...
    if (registrations.length > 0) {
      ...
      for (let i = 0; i < json.length && i < 2e3; i += 500) {
        meta[`reg_${Math.floor(i / 500) + 1}`] = json.slice(i, i + 500);
      }
    }
    ...
    return jsonError(500, err.message || "Checkout failed");
  }
};
